...
export default function ReceptionPage() {
  ...
  const arrive = useMutation({
    ...
    onSuccess: () => { toast("success", "Patient added to queue."); refresh(); },
    onError: (e) => toast("error", apiError(e)),
  });
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <Tab id="queue" label="Waiting Queue" count={queue.length} />
        <Tab id="appointments" label="Today's Appointments" count={appts.length} />
        <Tab id="completed" label="Completed" count={completed.length} />
      ...
      {tab === "queue" && (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {queue.length === 0 && <div className="card text-slate-400">The queue is empty.</div>}
          {queue.map((v) => (
            <div key={v.id} className="card">
              ...
              <div className="mt-2 text-xs text-slate-400">
                Arrived: {v.arrival_time ? new Date(v.arrival_time).toLocaleTimeString() : "—"}
              </div>
              {v.medical_summary && (
        ...
        <div className="card overflow-x-auto p-0">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-700/50">
              <tr>{["Time", "Patient", "Doctor", "Status", ""].map((h) => <th key={h} className="px-4 py-2 text-start font-medium">{h}</th>)}</tr>
            </thead>
            <tbody>
              {appts.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-slate-400">No appointments today.</td></tr>}
              {appts.map((a) => (
                <tr key={a.id} className="border-t border-slate-100 dark:border-slate-700">
                  ...
                  <td className="px-4 py-2"><span className={`badge ${STATUS_COLOR[a.status] || "bg-slate-100 text-slate-700"}`}>{a.status}</span></td>
                  <td className="px-4 py-2">
                    ...
                    {a.status === "ARRIVED" && <span className="text-xs text-amber-600">In queue</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {tab === "completed" && (
        <div className="card overflow-x-auto p-0">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-700/50">
              <tr>{["#", "Patient", "Doctor", "Diagnosis"].map((h) => <th key={h} className="px-4 py-2 text-start font-medium">{h}</th>)}</tr>
            </thead>
            <tbody>
              {completed.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-slate-400">No completed visits today.</td></tr>}
              {completed.map((v) => (
