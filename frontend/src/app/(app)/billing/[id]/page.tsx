    ...
    <div>
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            {canEdit && inv.status !== "PAID" && <button className="btn-ghost" onClick={() => setEditing(true)}>Edit</button>}
            {canPay && Number(inv.balance) > 0 && <button className="btn-primary" onClick={() => setPayOpen(true)}>Record Payment</button>}
          </div>
        }
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="card mt-4 p-0">
            <div className="border-b border-slate-200 px-4 py-2 font-semibold dark:border-slate-700">Payment History</div>
            <table className="w-full text-sm">
              <tbody>
                {payments.length === 0 && <tr><td className="p-4 text-slate-400">No payments yet.</td></tr>}
                {payments.map((p) => (
                  <tr key={p.id} className="border-t border-slate-100 dark:border-slate-700">
                    <td className="px-4 py-2">{new Date(p.paid_at).toLocaleString()}</td>
                    <td className="px-4 py-2">{p.method}</td>
                    <td className="px-4 py-2">{fmt(p.amount)}</td>
                    <td className="px-4 py-2 text-end"><a className="text-brand-600 hover:underline" href={`${API_URL}/payments/${p.id}/receipt/`} target="_blank" rel="noreferrer">Receipt ↗</a></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {payOpen && (
        <PaymentModal invoiceId={inv.id} balance={Number(inv.balance)} onClose={() => setPayOpen(false)}
          onDone={() => { setPayOpen(false); refresh(); toast("success", "Payment recorded."); }}
  ...
  return (
    <Modal open title="Record Payment" onClose={onClose}>
      <form onSubmit={(e) => { e.preventDefault(); pay.mutate(); }} className="space-y-3">
        <div><label className="label">Amount</label><input className="input" value={amount} onChange={(e) => setAmount(e.target.value)} /></div>
        <div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button className="btn-primary" disabled={pay.isPending}>Save</button>
        </div>
      </form>
    </Modal>
  );
...
function InstallmentPanel({ invoice, plan, canInstall, canPay, refresh, toast }: any) {
  ...
  const createPlan = useMutation({
    ...
    onSuccess: () => { refresh(); toast("success", "Installment plan created."); },
  ...
  if (!plan) {
    ...
    return (
      <div className="card">
        <h3 className="mb-2 font-semibold">Installment Plan</h3>
        <div className="flex items-end gap-2">
          <div><label className="label">Number</label><input type="number" min={2} className="input w-20" value={count} onChange={(e) => setCount(Number(e.target.value))} /></div>
          <button className="btn-ghost" disabled={createPlan.isPending} onClick={() => createPlan.mutate()}>Create</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="card p-0">
      <div className="border-b border-slate-200 px-4 py-2 font-semibold dark:border-slate-700">Installments</div>
      <table className="w-full text-sm">
        <tbody>
            ...
            <tr key={ins.id} className="border-t border-slate-100 dark:border-slate-700">
              ...
              <td className="px-4 py-2 text-end">
                {canPay && ins.status !== "PAID" && (
                  <button className="btn-ghost px-2 py-1 text-xs" onClick={() => setPayFor(ins)}>Pay</button>
                )}
              </td>
            </tr>
          ))}
      ...
      {payFor && (
        <PaymentModal invoiceId={invoice.id} balance={Number(payFor.amount) - Number(payFor.paid_amount)} installmentId={payFor.id}
          ...
          onDone={() => { setPayFor(null); refresh(); toast("success", "Installment paid."); }}
